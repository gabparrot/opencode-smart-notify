...
Notify only when a human still needs to look: pending permission, question, real error, or a finished turn. Stay silent when auto-approve, a prior reply, focus, or cancel already handled it. Retract a request notification that is no longer relevant (replied or window focused). Do not retract an idle popup just because the session became busy again.
